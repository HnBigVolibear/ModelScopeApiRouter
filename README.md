...
- 🖼️ **多类型支持**：支持 chat、vision、txt2img、img2img
...
首次运行后，编辑 `refactored_router/router_data/config.json`，可按分类维护模型列表：
...
├── refactored_router/
│   ├── default_data/            # 首次启动初始化模板
│   │   ├── api_keys.json        # 默认 API Keys 模板
│   │   └── config.json          # 默认模型配置模板
│   ├── router_data/             # 数据持久化目录
│   │   ├── .gitkeep
...
- 重构多 Key / 多模型路由架构
- 健康分调度 + 熔断冷却机制
- 支持 chat / vision / txt2img / img2img 四类模型
